...
# and selects all, except for the first three (they contain info that is now in the new string)
# meta is a DATAFRAME! -> read.xlsx outputs a dataframe
...
# this function basically pulls the sr15 data from world data or regions
# also it decides whether to make a new pull of the data from excel
# or take the existing ones in the environment
# also it combines model-scenario into one column and match with SBTi scenario file
...
# just pulls Energy Technology Perspectives (ETP) data in to a df
...
# I don't fully understand each single line of code, but I think knowing
# what it does is sufficient for now as its merely for interpolating datapoints
...
# basically just creates new variables as ratios of different indicators.
# i.e. GDP, cement etc.
...
# needed to reassign the meta-data to the new model-scenario combinations
...
# to draw the data and run regressions, get slopes etc.
...
# var0 is 'parent var' and above code just defines lists of variable names
# those are the variables that will be used in the regressions
...
# gets the varnames, using above defined formula
